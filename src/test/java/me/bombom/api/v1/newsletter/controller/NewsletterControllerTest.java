...
class NewsletterControllerTest extends ControllerTestSupport {
    ...
    void getNewsletters_success() throws Exception {
        ...
        GetNewsletterSummaryResponse response1 = new GetNewsletterSummaryResponse(
                1L, "뉴스레터1", "img1", "테크", "매주", 10, "INACTIVE", "ACTIVE", null);
        GetNewsletterSummaryResponse response2 = new GetNewsletterSummaryResponse(
                2L, "뉴스레터2", "img2", "경제", "격주", 20, "INACTIVE", "ACTIVE", null);
    ...
    @Test
    @DisplayName("뉴스레터 상태 변경 성공")
    void updateStatus_success() throws Exception {
        // given
        UpdateNewsletterStatusRequest request = new UpdateNewsletterStatusRequest(
                NewsletterPublicationStatus.SUSPENDED, LocalDate.of(2025, 11, 1));

        doNothing().when(newsletterService).updateStatus(1L, request);

        // when & then
        mockMvc.perform(patch("/admin/api/v1/newsletters/{id}/status", 1L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andDo(print())
    ...
    @Test
    @DisplayName("status가 null이면 400 에러")
    void updateStatus_nullStatus() throws Exception {
        // given
        UpdateNewsletterStatusRequest request = new UpdateNewsletterStatusRequest(null, null);
    ...
    @Test
    @DisplayName("존재하지 않는 뉴스레터 상태 변경 시 404 에러")
    void updateStatus_notFound() throws Exception {
        ...
        doThrow(new CIllegalArgumentException(ErrorDetail.ENTITY_NOT_FOUND)
                .addContext("newsletterId", 0L))
                .when(newsletterService).updateStatus(any(Long.class), any(UpdateNewsletterStatusRequest.class));

        // when & then
        mockMvc.perform(patch("/admin/api/v1/newsletters/{id}/status", 0L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andDo(print())
                .andExpect(status().isNotFound());

...
2. **신규 배정**: 대상 브랜치의 열린 PR 중 ① OPEN 배정 기록 없음 ② 수동 지정된 리뷰어 없음 ③ 아직 리뷰가 없음 ④ 제외 라벨(기본 `No Review`) 없음 인 PR에 대해:
   ...
   - GitHub 리뷰어 지정 → `review_assignment` INSERT(기한 = `review_setting.deadline_hours`, 기본 96h=4일) → `last_assigned_at` 갱신 → Discord 멘션 알림
...
- **리뷰어 추가/삭제/이름 변경**: admin 대시보드(`/reviewers`)에서 관리. Discord 멘션을 받으려면 `.github/notify_ids.json`에도 ID 추가 필요
- **기한/제외 라벨 변경**: admin 대시보드 설정에서 수정 (`review_setting` 테이블, 기본: 4일 / `No Review`)
- **특정 PR 자동 배정 제외**: PR에 제외 라벨(기본 `No Review`)을 붙이면 배정하지 않음
